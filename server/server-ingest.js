...
// HISTORY: list events for a camera (motion/doorbell history)
app.get("/api/ring/cameras/:id/history", async (req, res) => {
  try {
    const cameraId = req.params.id;

    // load camera from cached list
    const cams = await listCameras();
    ...
    const kind = req.query.kind; // "motion" | "ding" | undefined
    ...
    if (since || until) {
      events = events.filter((e) => {
        const ts =
          e?.created_at || e?.createdAt || e?.timestamp || e?.occurred_at || e?.answered_at;
    ...
    for (const e of events) {
      ...
      // Playable criteria (what you asked)
    ...
    /*const slim = events.map((e) => {
      ...
      const dingIdStr =
        ...
        e?.dingIdStr ??
        e?.event_id ??
        (e?.ding_id != null ? String(e.ding_id) : null);

      const dingId =
        e?.ding_id ?? e?.dingId ?? e?.data?.event?.ding?.id;

      return {
        id: e?.id ?? dingId ?? e?.recording_id ?? e?.recordingId,
        dingIdStr,
        dingId,
        kind: e?.kind ?? e?.type ?? e?.eventType,
        created_at: e?.created_at ?? e?.createdAt ?? e?.timestamp,
        doorbot_id: e?.doorbot_id ?? e?.doorbotId,
        answered: e?.answered,
        raw: e,
      }
    }).filter(x => x.dingIdStr || x.dingId);*/
...
app.get("/api/ring/cameras/:id/recordings/:eventId", async (req, res) => {
  try {
    ...
    // Optional: quick guard so we don't spam Ring for non-recorded activity events
    // We do a small getEvents() and search for this ding_id_str to confirm it's playable.
    try {
      const cams = await listCameras();
      const cam = cams.find((c) => String(c.id) === String(cameraId));
      ...
      // If this guard fails, we still attempt getCameraRecording.
    }
    ...
    // Return cleaner message for common Ring not-found
